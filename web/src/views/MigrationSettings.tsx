import { Button, Confirm, Text, TextField } from '@concrnt/ui'
...
export const MigrationSettingsView = () => {
    ...
    const canMigrate = client.api.authProvider.canSignMaster() && client.api.authProvider.canSignSub()
    ...
    const openRegistrationPage = async () => {
        ...
        try {
            ...
            window.open(
                `https://${destination}/register?document=${encodedDoc}&signature=${signature}`,
                '_blank',
                'noopener,noreferrer'
            )
    ...
    const switchSession = async () => {
        // webセッションのsubkey文字列はドメインを含むため移住先に書き換える
        ...
        const subKey = readStored('SubKey')
        if (subKey) {
            const parts = subKey.split(' ')
            if (parts.length >= 3) {
                parts[2] = `${client.ccid}@${destination}`
                localStorage.setItem('SubKey', parts.join(' '))
            }
        }
        ...
        },
        {
            ...
            content: (
                <>
                    ...
                    <Text style={{ opacity: 0.8 }}>{t('step2.backupNote')}</Text>
