...
export class Message<T> implements Document<T> {
    ...
    static async load<T>(client: Client, uri: string, hint?: string): Promise<Message<T> | null> {
        ...
        if (client.ccid) {
            try {
                message.ownAssociations = (
                    await client.api.getAssociationsAll(uri, { author: client.ccid }, authorHint)
                ).map((sd) => Association.fromSignedDocument(sd))
        ...
        message.associationCounts = await client.api.getAssociationCounts(uri, undefined, authorHint).catch(() => ({}))
        message.reactionCounts = await client.api
            .getAssociationCounts(uri, Schemas.reactionAssociation, authorHint)
            .catch(() => ({}))
