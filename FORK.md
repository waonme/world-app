...
`.github/workflows/upstream-status.yaml` は毎週 `origin/main` と `concrnt/world-app` の差を確認し、未取り込みcommitがあれば失敗として通知します。自動mergeは行いません。
...
| F-005 | Retained / VPS operator | 本番はfreshかつcleanな worktreeからテスト済み `main` の正確な SHA をビルド・配備し、非fast-forward、build / rollout / smoke test 失敗時に成功 SHA を進めない。変更前DeploymentとSHAをtransaction journalへ保存し、失敗・TERM・次回起動時にはstrategy、template、image、probe、sidecarをまとめて復元する。state欠落、取得元不一致、retry backoffはfail-closedに扱う。 | `ops/vps-deployer/deploy.sh`, `deploy-lib.sh`, `test-deploy.sh`, `world-app-vps-deploy.service`, `web/Dockerfile`, `/cc-info` | `8f14d09..85e58db`; deployer safety tests、static review、本番 smoke |
