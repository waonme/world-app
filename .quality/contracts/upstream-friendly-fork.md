...
- Native backup cancellation/failure must not unlock destructive account removal. On web, removal unlocks only after recovery data is constructed and a browser-managed download is initiated; completion or cancellation after that handoff is not observable by the page and is not guaranteed.
...
| QC-002 | User, word, and timeline mute support expiry, saved-entry home/global scope and placeholder policy, reroutes-only mode, direct author-mute actions, normal-notification rendering, actor filtering for aggregated notifications, and envelope/target filtering for timeline new arrivals. | Muted content leaks through a documented covered surface, the direct mute action disappears from either client, an author action is assigned to the wrong relationship, or unrelated content is hidden. | Load active, expired, malformed, scoped, and overlapping mute entries, including outer/target association combinations, then evaluate the own/foreign author action matrix. | Pure mute-policy/combination/action tests plus app/web type/build and action/rendering anchor checks. | Deterministic priority `block > user > word > timeline`; any matched block/hard-hide boundary remains unrevealable; expired entries never match when evaluated; both clients offer delete for self and mute for another author. | Unit / Integration | Yes |
...
| QC-007 | Logout, restore, subkey, follow, bridge-setting, and retained-domain recovery preserve the `dev` safety fixes. | Logout provisions credentials, partial settings overwrite good state, or recovery material is deleted before the platform-specific backup success boundary. | Exercise old/new follow keys, logged-out/v1 migration sessions, bridge load/save states, logout retention, native backup success/cancel/failure ordering, and web download handoff anchors. | Pure fork-policy tests, component-use/static anchors, and app/web build checks. | No automatic reenrollment after ordinary logout, no settings write before a valid load state, and save failure rolls back. Native reset unlocks only after the save promise resolves; web reset unlocks only after recovery data construction and browser-managed download initiation. | Unit / Integration | Yes |
| QC-008 | Upstream synchronization is a reviewable merge into production, never an implicit reset or lookalike/re-written/replaced fetch. | `main` or any unrelated local ref is changed by fetch, production is force-reset to upstream, a wrong/effectively rewritten repository, replacement tree, hidden index state, transport override, or local/global merge policy is trusted, or production deploys before fork gates. | Run the documented sync command/workflow against accepted remotes, lookalike URLs, symbolic or filesystem-symlink fetch destinations, direct/include/includeIf URL rewrite and local HTTP/TLS config, hostile proxy/CA environment, replace/graft/shallow state, ignored and user-global attributes plus merge drivers, diff rename policy, assume-unchanged/skip-worktree entries, concurrently moved refs, and ahead/behind local main. | Remote/object-provenance tests, fetch-destination and full-ref invariance checks, transport-environment isolation, captured-main attribute source with external attributes disabled, exact merge parents, unchanged local/remote main, generated integration branch/PR, and required status checks. | Local main must already equal origin/main; each canonical fetch may change only its direct remote-tracking destination; canonical GitHub HTTPS uses system trust only; no URL/object/transport rewrites, hidden index entries, external merge policy, or force push; production changes only after reviewed gates. | Unit / Static / Runtime | Yes |
...
- INV-012: Sync fetches use canonical GitHub HTTPS URLs with global/system/command config plus proxy/custom-CA overrides disabled; reject symbolic/logically redirected fetch destinations, repository-local URL/HTTP/object/diff/merge-policy rewrites, and hidden index entries; snapshot all refs outside the intended destination across every fetch; pin merge attributes to the pre-fetch captured `main`; verify exact merge parents; and leave local/remote `main` unchanged.
...
| Symbolic or filesystem-symlink fetch destination redirects an update into a local branch | Silent production-branch rewrite | advanced-origin, upstream, and physical-ref redirect fixtures plus full-ref snapshots | pre-fetch direct-ref/path gate + post-fetch invariance check | Yes | independent verifier |
...
22. Given `origin/main` or `upstream/main` as a Git symbolic ref or filesystem-symlink path into a local branch, synchronization is wrong if it fetches, moves any non-destination ref, or captures the moved branch as its trusted baseline.
...
- After a web recovery export is handed to a browser-managed download, the page cannot observe the browser or OS save dialog. Completion, the final filesystem destination, and cancellation after that handoff are not guaranteed; native save completion/cancellation remains covered by QC-007.
