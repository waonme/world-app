...
本統合の固定点は upstream `9e2fc5244ea23dfe648dc6ab96465728532f2dae`、旧仕様の参照点は `origin/dev` の `52dd8f82671a1eb0a083e642f71e71087ef108e1` です。
...
| F-004 | Retained / waonme fork | logout は新しい subkey を勝手に発行せず、Domain と master key / mnemonic を保持する。logged-out session は明示的な再登録でのみ復旧し、完全削除は壊れた値を含む recovery material の backup 成功後にだけ許可する。保存キャンセル・失敗は成功として扱わない。 | `worldlib/src/forkCompatibility.ts`, `web/src/contexts/Client.tsx`, `web/src/lib/subkey.ts`, `web/src/lib/v1storage.ts`, `web/src/components/EmergencyKit.tsx`, 両方の `ResetSessionButton.tsx`, `app/src/components/BackupKeyButton.tsx`, `app/src/views/Welcome.tsx` | [PR #15](https://github.com/waonme/world-app/pull/15) (`b22e45a`, `53334a0`, `2066592`, `492acb8`); fork policy tests、recovery anchors、app/web build |
| F-005 | Retained / VPS operator | 本番はfreshかつcleanな worktreeからテスト済み `main` の正確な SHA をビルド・配備し、非fast-forward、build / rollout / smoke test 失敗時に成功 SHA を進めない。変更前DeploymentとSHAをtransaction journalへ保存し、resourceVersion付きpatchとtransaction固有のpostpatch状態が一致する場合だけ、失敗・TERM・次回起動時にmetadata、strategy、template、image、probe、sidecarをまとめて復元する。外部更新、state欠落、取得元不一致、retry backoffはfail-closedに扱う。 | `ops/vps-deployer/deploy.sh`, `deploy-lib.sh`, `test-deploy.sh`, `world-app-vps-deploy.service`, `web/Dockerfile`, `/cc-info` | `8f14d09..85e58db`; deployer safety tests、static review、本番 smoke |
...
1. clean なローカル `main` で `scripts/prepare-upstream-sync.sh` を実行します。スクリプトは `origin/main` と一致しない状態、lookalike remote、localの `url.*.insteadOf` を拒否し、正規GitHub URLから取得した固定 upstream SHA を含む統合ブランチを作ります。
